...
| studioBlobvault | object | `{"image":{"repository":"nginx","tag":"1.29.5-alpine"},"podSecurityContext":{},"securityContext":{},"service":{"port":80}}` | Studio: Additional service to expose the blobvault files generated by the worker It is enabled automatically if the worker is scaled to 1 replica and no bucket is configured |
| studioBlobvault.image | object | `{"repository":"nginx","tag":"1.29.5-alpine"}` | Image to use for the blobvault service |
| studioBlobvault.image.repository | string | `"nginx"` | Image repository |
| studioBlobvault.image.tag | string | `"1.29.5-alpine"` | Image tag |
...
| studioPypiCache | object | `{"affinity":{},"enabled":false,"image":{"repository":"nginx","tag":"1.29.5-alpine"},"nodeSelector":{},"podSecurityContext":{},"securityContext":{},"service":{"port":8080},"storage":{"persistentVolumeClaim":{"claimName":"pypi-cache","storageClass":""},"size":"100Gi","type":"pvcRWO"},"tolerations":[]}` | Studio: Additional service to cache PyPI packages |
| studioPypiCache.enabled | bool | `false` | PyPI cache enabled |
| studioPypiCache.image | object | `{"repository":"nginx","tag":"1.29.5-alpine"}` | Image to use for the pypi-cache service |
| studioPypiCache.image.repository | string | `"nginx"` | Image repository |
| studioPypiCache.image.tag | string | `"1.29.5-alpine"` | Image tag |
